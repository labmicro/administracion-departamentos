...
const EditarDocenteAsignatura = () => {
  const router = useRouter(); // Usa useRouter para obtener la información de enrutamiento
  const { idAsignatura, idDocenteAsignatura } = router.query; // Obtén idAsignatura e idDocenteAsignatura de los parámetros de la ruta
  console.log("params", idAsignatura, idDocenteAsignatura);
  ...
  const [asignaturaDocente, setAsignaturaDocente] = useState<AsignaturaDocente>();
  const [currentUrlAsigDte, setCurrentUrlAsigDte] = useState<string>(`http://127.0.0.1:8000/facet/asignatura-docente/${idDocenteAsignatura}/`);
  ...
  useEffect(() => {
    fetchDataAsignaturaDocente(currentUrlAsigDte);
  }, [currentUrlAsigDte, idDocenteAsignatura]);

  const fetchDataAsignaturaDocente = async (url: string) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/facet/asignatura-docente/${idDocenteAsignatura}/`);
      setAsignaturaDocente(response.data.results);
      SetNroResolucion(response.data.resolucion.nresolucion);
      SetDni(response.data.docente.persona.dni);
      setNombreAsig(response.data.asignatura.nombre);
      setNombre(response.data.docente.persona.nombre);
      SetApellido(response.data.docente.persona.apellido);
      setObservaciones(response.data.observaciones);
      // Convierte las fechas a objetos Dayjs
      setFechaInicio(dayjs(response.data.fecha_de_inicio).utc());
      setFechaFin(dayjs(response.data.fecha_de_vencimiento).utc());
      setAsignatura(response.data.asignatura);
      setResolucion(response.data.resolucion);
      setPersona(response.data.docente);
  ...
  const fetchData = async (url: string) => {
    try {
      ...
      console.log("asig", response.data.results);
  ...
  const handleConfirmModal = () => {
    router.push('/dashboard/asignaturas/docentes/'); // Cambia a usar el enrutamiento de Next.js
  };
  ...
  const totalPagesDepto = Math.ceil(totalItemsDepto / pageSizeDepto);

  const crearNuevoJefeDepartamento = async () => {
    const formatFecha = (date: dayjs.Dayjs | Date | null | undefined) => {
      if (date) {
        const dayjsDate = dayjs(date);
        return dayjsDate.isValid() ? dayjsDate.toISOString() : null;
      }
      return null;
    };
    ...
    try {
      const response = await axios.put(`http://127.0.0.1:8000/facet/asignatura-docente/${idDocenteAsignatura}/`, nuevoDocenteAsignatura, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
      handleOpenModal('Bien', 'Se modifico el docente en asignatura con éxito', handleConfirmModal);
    } catch (error: unknown) {
      console.error(error);
            ...
            <Dialog open={openResolucion} onClose={handleClose} maxWidth="md" fullWidth>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px' }}>
                <Button
                  ...
                  color="primary"
                  onClick={() => {
                    prevUrlResolucion && setCurrentUrlResolucion(prevUrlResolucion);
          ...
          <Grid item xs={4}>
            ...
            <Dialog open={openPersona} onClose={handleClose} maxWidth="md" fullWidth>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px' }}>
                ...
                <Typography variant="body1">
                  Página {currentPagePersona} de {totalPagesPersona}
                </Typography>
                <Button
                  variant="contained"
                  color="primary"
          ...
          <Grid item xs={4}>
            ...
            <Dialog open={openDeptos} onClose={handleClose} maxWidth="md" fullWidth>
              ...
              <List>
                {asignaturas.map((asignaturaFiltrado) => (
                  <ListItem
                    ...
                    button
                    onClick={() => { setAsignatura(asignaturaFiltrado); setNombreAsig(asignaturaFiltrado.nombre); }}
                    style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px' }}>
                ...
                <Typography variant="body1">
                  Página {currentPageDepto} de {totalPagesDepto}
                </Typography>
                <Button
                  variant="contained"
                  color="primary"
