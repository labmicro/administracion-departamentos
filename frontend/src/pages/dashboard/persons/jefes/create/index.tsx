...
const CrearJefe = () => {
  ...
  const crearNuevoJefeDepartamento= async () => {
    ...
    try {
      ...
      const existeRegistro = await axios.get(`http://127.0.0.1:8000/facet/api/v1/jefes/${idPersona}`);
      ...
      if (axios.isAxiosError(error) && error.response && error.response.status === 404) {
        ...
        const response = await axios.post('http://127.0.0.1:8000/facet/api/v1/jefes/', nuevoJefe, {
          headers: {
