...
const EditarJefe : React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/facet/api/v1/jefes/${idPersona}`);
  ...
  const edicionDepartamentoJefe = async () => {
    ...
    try {
      const response = await axios.put(`http://127.0.0.1:8000/facet/api/v1/jefes/${idPersona}/`, jefeEditado, {
        headers: {
...
try {
  const response = await axios.delete(`http://127.0.0.1:8000/facet/api/v1/jefes/${idPersona}/`,{
    headers: {
      'Content-Type': 'application/json', // Ajusta el tipo de contenido según sea necesario
    },
  });
