...
const EditarNoDocente : React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/facet/api/v1/nodocentes/${idPersona}`);
  ...
  useEffect(() => {
    if (NoDocente) {
      setEstado(String(NoDocente.estado))
  ...
  const edicionNoDocente = async () => {
    ...
    try {
      const response = await axios.put(`http://127.0.0.1:8000/facet/api/v1/nodocentes/${idPersona}/`, noDocenteEditado, {
        headers: {
...
try {
  const response = await axios.delete(`http://127.0.0.1:8000/facet/api/v1/nodocentes/${idPersona}/`,{
    headers: {
      'Content-Type': 'application/json', // Ajusta el tipo de contenido según sea necesario
    },
  });
  handleOpenModal('No Docente Eliminado', 'La acción se realizó con éxito.');
} catch (error) {
  // console.error('Error al hacer la solicitud PUT:', error);
  handleOpenModal('Error','NO  se pudo realizar la acción.');
...
<Paper elevation={3} style={{ padding: '20px', marginTop: '20px' }}>
  ...
  <Grid container spacing={2}>
      ...
      <Grid item xs={12} marginBottom={2}>
        <Button variant="contained" onClick={edicionNoDocente}>
          Editar
        </Button>
