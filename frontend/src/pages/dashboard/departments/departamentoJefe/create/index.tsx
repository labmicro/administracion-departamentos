...
const [personas, setPersonas] = useState<Jefe[]>([]);
...
const [currentUrlPersona, setCurrentUrlPersona] = useState<string>('http://127.0.0.1:8000/facet/jefe/');
...
const fetchDataResoluciones = async (url: string) => {
  ...
  const handleFilterPersonas = (filtro: string) => {
    // Lógica para filtrar las resoluciones según el término de búsqueda
    const personasFiltro = personas.filter((jefe) =>
      jefe.persona.dni.includes(filtro)||
      (jefe.persona.legajo ?? '').includes(filtro)||
      jefe.persona.nombre.toLowerCase().includes(filtro.toLowerCase())||
      jefe.persona.apellido.toLowerCase().includes(filtro.toLowerCase())
    );
  ...
  const crearNuevoJefeDepartamento= async () => {
    ...
    console.log(departamento,persona,resolucion)
...
<Paper elevation={3} style={{ padding: '20px', marginTop: '20px' }}>
  ...
  <List>
    {departamentos.map((departamentoFiltrado) => (
      <ListItem
        key={departamentoFiltrado.id}
        button
        onClick={() => { setDepartamento(departamentoFiltrado); setNombreDepto(departamentoFiltrado.nombre); }}
        style={{
          backgroundColor: departamentoFiltrado.id === departamento?.id ? '#4caf50' : 'inherit',
          color: departamentoFiltrado.id === departamento?.id ? 'white' : 'inherit'
        }}
      >
